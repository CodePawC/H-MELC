...
function isWebSocketConnected() {
  return APIServiceState.websocket && APIServiceState.websocket.readyState === WebSocket.OPEN;
}
...
function sendMsg(msg, callback) {
  ...
  const performSend = (handler) => {
    if (!isWebSocketConnected()) {
      ...
      delete APIServiceState.MessageList[msg.apiName]; // 清理
      return;
    }
    ...
    handler.timerId = setTimeout(function () {
      ...
      const currentHandler = APIServiceState.MessageList[msg.apiName];
      if (currentHandler) {
        const err = new Error("打印服务消息接收超时");
        handleError(currentHandler, err);
        delete APIServiceState.MessageList[msg.apiName];
      }
    }, APIServiceState.timeout_duration);

    // 发送数据
    for (var i = 0; i < tryTimes; i++) {
      APIServiceState.websocket.send(data);
      return;
    }
  };
  ...
  if (callback && typeof callback === 'function') {
    // 模式 A: 回调模式 (兼容旧代码)
    APIServiceState.MessageList[msg.apiName] = {
      type: 'cb',
      func: callback,
      timerId: null // 预留计时器ID槽位
    };
    performSend(APIServiceState.MessageList[msg.apiName]);
  } else {
    // 模式 B: Promise 模式 (新特性)
...
function getInstance(onServiceConnected, onNotSupportedService, onServiceDisconnected) {
  ...
  const connect = () => {
    ...
    if ('WebSocket' in window) {
      APIServiceState.websocket = new WebSocket('ws://127.0.0.1:37989');
      if ('binaryType' in WebSocket.prototype) {
        APIServiceState.websocket.binaryType = 'arraybuffer';

      }

      if ('timeout' in WebSocket.prototype) {
        APIServiceState.websocket.timeout = 5000;
      }


      APIServiceState.websocket.addEventListener('open', (_) => {
        isConnected = true;
      ...
      APIServiceState.websocket.addEventListener('error', (event) => {
        isConnected = false;
      ...
      APIServiceState.websocket.addEventListener('close', (event) => {
        isConnected = false;
        APIServiceState.ackJsonData = '';
      ...
      APIServiceState.websocket.addEventListener('message', (event) => {
        readCallback(event);
      });
...
function readCallback(event) {
  ...
  APIServiceState.ackJsonData = callBackInfo;


  if (isJSON(APIServiceState.ackJsonData)) {
    var arrParse = JSON.parse(APIServiceState.ackJsonData);
    ...
    if (APIServiceState.MessageList[apiName]) {
      ...
      if (apiName === 'commitJob' && arrParse.resultAck && arrParse.resultAck.info !== 'commitJobApi Success!') {
        ...
        const handler = APIServiceState.MessageList[apiName];
        // 1. 立即清除计时器
        if (handler.timerId) {
        ...
        delete APIServiceState.MessageList[apiName];
        // 3. 执行回调
        if (handler.type === 'cb' && typeof handler.func === 'function') {
          // 情况 A: 兼容旧模式，直接回调
    ...
    if (arrParse.apiName == 'commitJob') {
      APIServiceState.jobListeners.forEach(listener => {
      ...
      APIServiceState.statusListeners.forEach(listener => {
        try { listener(arrParse); } catch (e) { console.error(e); }
      });
    }
...
function scanWifiPrinter(callbackFunction = null) {
  APIServiceState.timeout_duration = 25000;
  //刷新设备时，关闭设备
  //closePrinter();
  var jsonObj = { apiName: 'scanWifiPrinter' };
  // [修改] 增加 return
  const result = sendMsg(jsonObj, callbackFunction);
  APIServiceState.timeout_duration = 10000;
...
function addJobListener(callbackFunction) {
  ...
  // 确保去重：先删除后添加
  APIServiceState.jobListeners.delete(callbackFunction);
  APIServiceState.jobListeners.add(callbackFunction);
...
function addPrinterStatusListener(callbackFunction) {
  if (typeof callbackFunction !== "function") return;
  APIServiceState.statusListeners.delete(callbackFunction);
  APIServiceState.statusListeners.add(callbackFunction);
  return callbackFunction;
}
...
function getListenersCount() {
  return {
    job: APIServiceState.jobListeners.size,
    status: APIServiceState.statusListeners.size
  };
}
...
function getResult(tryTime, apiName, errInfo) {
  ...
  let result = {};
  while (tryTimes--) {
    if (!isJSON(APIServiceState.ackJsonData)) continue;
