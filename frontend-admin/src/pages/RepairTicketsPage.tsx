...
import {
  repairsFiltersFromSearchParams,
  searchParamsFromFilters,
} from '../lib/repairListUrlSync'

/** 对齐 docs/05 §维修管理 · 报修工单；接口 docs/06 §二·2 */
...
export function RepairTicketsPage() {
  ...
  const [form, setForm] = useState(() => repairsFiltersFromSearchParams(searchParams))
  const [active, setActive] = useState(() => repairsFiltersFromSearchParams(searchParams))
  ...
  /** 深链：路由查询与列表筛选一致（如自设备详情跳转 `?asset_id=`） */
  useEffect(() => {
    const fromUrl = repairsFiltersFromSearchParams(searchParams)
    setForm(fromUrl)
    setActive(fromUrl)
    setPage(1)
  }, [searchParams])
  ...
  useEffect(() => {
    ...
    const q: RepairListQueryParams = {
      page,
      page_size: pageSize,
      ...(active.order_status?.trim() ? { order_status: active.order_status.trim() } : {}),
  ...
  function filterSubmit(ev: FormEvent) {
    ev.preventDefault()
    const next = searchParamsFromFilters(form)
  ...
  return (
    <div className="page">
      ...
      <p className="muted tiny">
        角色需含报修可读权限（RBAC_REPAIR_READ）；筛选可与地址栏查询同步（例如自设备详情跳转 <code>?asset_id=</code>）。
      </p>
      ...
      <section className="wf-panel repair-filter-panel">
        ...
        <form className="repair-filter-form" onSubmit={filterSubmit}>
          <label className="tiny">
            工单状态
            <input
              placeholder="如 PENDING_DISPATCH、ASSIGNED、CLOSED"
          ...
          <label className="tiny">
            维修工程师（H-UMDG）
            <div style={{ display: 'flex', gap: 6 }}>
              <input readOnly placeholder="从 H-UMDG 选择人员" value={engineerLabel || form.assigned_engineer_id || ''} />
